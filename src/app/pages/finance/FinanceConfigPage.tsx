...
export function FinanceConfigPage() {
  ...
  // Numbering lives in NumberingConfigPanel, which owns its own state and
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Module numbering. Replaces a hand-rolled copy of this panel that
          filtered with `cfg.module.startsWith("Finance")` — the Finance
          sequences are Expense, Income, Budget, Claim, Payment, JournalEntry,
          PayrollRun, Accrual, Transaction, FinanceApproval and ScheduledPosting,
          so only FinanceApproval ever matched and the other ten could not be
          configured. The shared panel selects by the `app` a sequence belongs
          to and persists to the server. */}
      {tab === "numbering" && (
        <NumberingConfigPanel app="finance" accent="indigo" />
      )}
